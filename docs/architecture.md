...
## 十、2026-08-28 未读红点修复与登录卡片居中

### 10.1 未读红点（群聊读取后不消失）

根因（测试定位）：

1. **Vite HMR 导致 zustand store 模块被重复求值**，同一页面出现多个 store 实例：
   消息写入一个实例、UI 读取另一个实例，未读计数对不上（红点不消失/不出现）。
   修复：全部 6 个 store（accounts/auth/chat/friend/call/ui）改为 `globalThis`
   单例，HMR 重复求值时复用同一实例。
...
回归：未读场景 6/6、通话 26/26、好友 12/12、管理后台 16/16、登录 12/12。

### 10.2 登录卡片居中

`web/src/styles/login.css` 中 `.lg-card` 由"右对齐（`margin-right: 9vw`）"
改为 `margin: auto`，桌面与移动端均水平/垂直居中。
